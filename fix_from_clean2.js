...
files.forEach(f => {
  ...
  // Let's use a very safe string replacement instead of regex!
  const validate1bBlock = `        if (step === "1b") {
          let otp = "";
          document
            .querySelectorAll("#signup-otp-container input")
  ...
  content = content.replace(validate1bBlock, '');
  
  // also for single quotes
  const validate1bBlock2 = `        if (step === '1b') {
          let otp = "";
          document
            .querySelectorAll("#signup-otp-container input")
            .forEach((i) => (otp += i.value));
  ...
  content = content.replace(validate1bBlock2, '');
