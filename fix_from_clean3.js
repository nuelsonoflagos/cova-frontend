...
files.forEach(f => {
  ...
  content = content.replace(/if\s*\(\s*(?:step|cur)\s*===\s*['"]1b['"]\s*\)\s*\{[\s\S]*?return\s+true;\s*\}/, '');

  // 7. Update nextStep
  content = content.replace(/if\s*\(\s*from\s*===\s*1\s*\)\s*\{[\s\S]*?\}\s*if\s*\(\s*from\s*===\s*['"]1b['"]\s*\)\s*\{[\s\S]*?\}/, `if (from === 1) { goToStep(2); return; }`);

  // For travel, there is a weird syntax error originally from 5d42526!
  // `if(from === 1){ goToStep(2); return; }` outside function nextStep
  content = content.replace(/\}\s*if\s*\(\s*from\s*===\s*1\s*\)\s*\{\s*goToStep\(2\);\s*return;\s*\}/, `if (from === 1) { goToStep(2); return; }\n      }`);
  // and we should also remove `cur === "1b"` block in travel
  content = content.replace(/if\s*\(\s*cur\s*===\s*['"]1b['"]\s*\)\s*\{[\s\S]*?return;\s*\}/, `if (cur === 'otp') {\n          document.getElementById("step-otp").classList.remove("active");\n          document.getElementById("step-6").classList.add("active");\n          currentStep = 6;\n          return;\n        }`);
  content = content.replace(/if\s*\(\s*cur\s*===\s*2\s*\)\s*\{\s*document\.getElementById\("step-2"\)\.classList\.remove\("active"\);\s*document\.getElementById\("step-1b"\)\.classList\.add\("active"\);\s*return;\s*\}/, `if (cur === 2) {\n          document.getElementById("step-2").classList.remove("active");\n          document.getElementById("step-1").classList.add("active");\n          return;\n        }`);

  // 8. Update prevStep for cur/current
  content = content.replace(/if\s*\(\s*current\s*===\s*['"]1b['"]\s*\)\s*\{[\s\S]*?return;\s*\}/, `if (current === 'otp') {\n          document.getElementById("step-otp").classList.remove("active");\n          document.getElementById("step-6").classList.add("active");\n          currentStep = 6;\n          return;\n        }`);
  ...
  // Fix nextStep in fire/group/property if they have from === 1b
  content = content.replace(/if\s*\(\s*from\s*===\s*['"]1b['"]\s*\)\s*\{[\s\S]*?return;\s*\}/, ``);
